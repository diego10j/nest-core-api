import { Body, Controller, Delete, Get, Param, Post, Query, Res, UploadedFile, UseInterceptors, Headers } from "@nestjs/common";
...
export class AdjuntoCorreoController {
    ...
    async downloadAdjunto(
        @Param('ide_adco') ide_adco: number,
        @Res() res: Response
    ) {
    ...
    async getAdjuntosPorReferencia(
        @Query('tipo') tipo: 'plantilla' | 'campania' | 'cola',
