...
export class CoreService {
  ...
  async getSeqTable(dto: SeqTableDto & HeaderParamsDto) {
    const seqTable: number = await this.dataSource.getSeqTable(
      `${dto.module}_${dto.tableName}`,
      dto.primaryKey,
      dto.numberRowsAdded,
    );
    return {
  ...
  async search(dtoIn: SearchTableDto & HeaderParamsDto) {
    ...
    const whereClause = dtoIn.columnsSearch
      .map((col, index) => `regexp_replace(unaccent(LOWER(${col})), '[^a-z0-9]', '', 'g') LIKE $${index + 1}`)
      .join(' OR ');
   ...
   */
  validateDTO = async <T extends ClassConstructor<any>>(dto: T, obj: Object) => {
  ...
  private toQuery(
    ...
    if (dto.operation === 'update') {
      ...
      if (dto.condition) updateQuery.where = dto.condition;
      else updateQuery.where = `${dto.primaryKey} = ${valuePrimaryKey}`;
      ...
      //  asigna valores update campos del core
      if (dto.primaryKey !== 'ide_empr') mapObject.set('ide_empr', ideEmpr);
      if (dto.primaryKey !== 'ide_sucu') mapObject.set('ide_sucu', ideSucu);
      ...
      if (dto.condition) deleteQuery.where = dto.condition;
      else deleteQuery.where = `${dto.primaryKey} = ${valuePrimaryKey}`;
