...
export class ProductosService extends BaseService {
    ...
    async getTrnProducto(dtoIn: TrnProductoDto & HeaderParamsDto) {
        ...
        saldo_movimientos AS (
            ...
            SELECT
                ...
                NULL AS ide_incci,
                '${dtoIn.fechaInicio}' AS fecha_trans_incci,
                NULL as ide_inbod,
                NULL as nombre_inbod,
                NULL AS NUM_DOCUMENTO,        
                'SALDO INICIAL AL ${getDateFormatFront(dtoIn.fechaInicio, true)} ' AS  nom_geper,
    ...
    compras AS (
        SELECT
            dci.ide_inarti,
            f_decimales(MAX(dci.cantidad_indci), ${decim_stock_inarti})::numeric AS max_cantidad_compra,
            f_decimales(MIN(dci.cantidad_indci), ${decim_stock_inarti})::numeric AS min_cantidad_compra
        FROM
            inv_det_comp_inve dci
            INNER JOIN inv_cab_comp_inve cci ON cci.ide_incci = dci.ide_incci
            INNER JOIN inv_tip_tran_inve tti ON tti.ide_intti = cci.ide_intti
    ...
    SELECT
        COALESCE(v.total_facturas, 0) AS total_facturas,
        COALESCE(v.max_cantidad_venta, 0) AS max_cantidad_venta,
        COALESCE(v.min_cantidad_venta, 0) AS min_cantidad_venta,
        v.primera_fecha_venta,
        v.ultima_fecha_venta,
        COALESCE(c.max_cantidad_compra, 0) AS max_cantidad_compra,
        COALESCE(c.min_cantidad_compra, 0) AS min_cantidad_compra
    FROM
        ventas v
        LEFT JOIN compras c ON c.ide_inarti = v.ide_inarti
    `);
        query.addIntParam(1, ide_inarti);
