...
export class TipoComprobanteEnum {
  ...
  static getCodigo(descripcion: string): string | null {
    const entry = Object.values(TipoComprobanteEnum.data).find((item) => item.descripcion === descripcion);
    return entry ? entry.codigo : null;
  }

  static getDescripcion(codigo: string): string | null {
    const entry = Object.values(TipoComprobanteEnum.data).find((item) => item.codigo === codigo);
