...
export class EstadoComprobanteEnum {
  ...
  static getCodigo(descripcion: string): number | null {
    const entry = Object.values(EstadoComprobanteEnum.data).find((item) => item.descripcion === descripcion);
    return entry ? entry.codigo : null;
  }

  static getDescripcion(codigo: number): string | null {
    const entry = Object.values(EstadoComprobanteEnum.data).find((item) => item.codigo === codigo);
