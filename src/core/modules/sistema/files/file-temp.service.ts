...
export class FileTempService implements OnModuleDestroy, OnApplicationShutdown {
  ...
  private readonly fileLifetime = 90 * 24 * 3600 * 1000; // 90 días
  ...
  private setupCleanupInterval(): void {
    ...
    this.cleanupOldFiles().catch((err) => console.error('Initial cleanup failed:', err));
        ...
        try {
          ...
          if (fileAge > this.fileLifetime) {
            deletionPromises.push(
              unlink(filePath).catch((err) => console.error(`Error deleting file ${filePath}:`, err)),
            );
          }
        } catch (err) {
