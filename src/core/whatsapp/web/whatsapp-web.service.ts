...
export class WhatsappWebService implements OnModuleInit {
  ...
  private setupEventHandlers(ideEmpr: string, instance: WhatsAppClientInstance, cuenta: AccountConfig) {
    ...
    client.on('message_ack', (message, ack) => {
      if (ack === 3) {
        // 3 indicates the message was read    message.fromMe &&
  ...
  private async unsafeSendMedia(
    dto: UploadMediaDto & HeaderParamsDto,
  ...
  private async unsafeSendLocation(dto: EnviarUbicacionDto & HeaderParamsDto): Promise<SendMessageResponse> {
    ...
    const chatId = formatPhoneNumber(dto.telefono);
    const location = new Location(dto.latitude, dto.longitude, { name: dto.name, address: dto.address });
  ...
  async getChats(dto: GetChatsDto & HeaderParamsDto) {
    ...
    const sortedChats = filteredChats.sort((a, b) => (b.lastMessage?.timestamp || 0) - (a.lastMessage?.timestamp || 0));
  ...
  async validateWhatsAppNumber(
    ...
    try {
      ...
      const tmpFormattedNumber = atIndex === -1 ? formattedNumber : formattedNumber.substring(0, atIndex);
      ...
      this.logger.error(`Error al validar número ${phoneNumber} para empresa ${ideEmpr}:`, error);
   ...
   */
  async getOrCreateProfilePicture(ideEmpr: string, contactId: string, response: Response) {
   ...
   */
  private async createProfilePictureFile(ideEmpr: string, contactId: string, filename: string) {
    try {
