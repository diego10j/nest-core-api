...
export function getSqlUpdate(query: UpdateQuery) {
  ...
  if (query.where) {
    ...
    throw new InternalServerErrorException(`[ERROR]: No existen condiciones en el UPDATE`);
  }
...
export function getSqlDelete(query: DeleteQuery) {
  ...
  if (query.where) {
    ...
    throw new InternalServerErrorException(`[ERROR]: No existen condiciones en el DELETE`);
  }
