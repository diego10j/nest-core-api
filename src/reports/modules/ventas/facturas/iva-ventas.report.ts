...
import { VentaMensualRow } from 'src/core/modules/ventas/facturas/dto/reporte-ventas-mensuales.dto';
...
// ─── Paleta neutral y profesional (misma que factura RIDE / proformas) ──────
const C = {
  ink: '#1a1d27',
  body: '#374151',
  muted: '#6b7280',
...
const sumar = (rows: VentaMensualRow[]) =>
  rows.reduce(
    (acc, r) => ({
...
const buildTabla = (rows: VentaMensualRow[], etiquetaVacio: string): Content => {
  if (rows.length === 0) {
    return {
      text: etiquetaVacio,
      italics: true,
...
export const ivaVentasReport = (data: IvaVentasRep, headerSection: Content): TDocumentDefinitions => {
