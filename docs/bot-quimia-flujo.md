...
- ✅ (2026-07-07, noche) **REGRESIÓN corregida: el mensaje de confirmación no llegaba al teléfono** — al renombrar el botón a "✅ Confirmar cotización" (22 caracteres) se superó el **límite de Meta de 20 caracteres por título de botón**, y Meta rechazaba el mensaje interactivo COMPLETO en silencio (YCloud acepta el envío y el fallo llega como status asíncrono `failed`; el dashboard sí lo mostraba porque se guarda en BD local). Los clientes quedaban esperando en el resumen sin poder confirmar → expiración → asesor. Fixes: título → "✅ Confirmar"; truncado automático a 20 codepoints en `sendInteractiveButtons` (red permanente); log ERROR `[Delivery] ... FALLÓ en Meta` en `processStatusUpdate` para que un rechazo de Meta nunca más pase inadvertido. **Regla: títulos de botones SIEMPRE ≤20 caracteres (codepoints).**
- ✅ (2026-07-07, noche) **Eliminado el "opt-out permanente" `tieneAgenteHumano`** — el chequeo por-mensaje que bloqueaba al bot para siempre si ALGUNA VEZ hubo un mensaje de agente humano en el chat. Quedó obsoleto y dañino desde el hand-off automático (07-04): hoy, cuando un humano escribe, `derivarPorAgenteHumano` pasa el chat a ASESOR al instante, así que `bot_activo_whcha=TRUE` ya garantiza que ningún humano intervino desde la última activación. El chequeo viejo anulaba la **activación manual**: caso real en PROD — el dueño activó manualmente su chat y el bot respondió "Chat tiene historial con agente humano — bot no responde". Nota: el chequeo de mensajes salientes humanos en la detección de chat NUEVO (Capa 1.5, `mensajeSalienteHumano`) se mantiene — ese tiene otro propósito (no auto-activar chats a los que el negocio escribió primero) y sigue vigente.
