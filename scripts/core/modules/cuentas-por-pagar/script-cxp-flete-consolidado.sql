...
-- Detalle: cada envío incluido en la factura consolidada y el valor
-- de la línea del XML que le corresponde (emparejado por GPT o a
-- mano en el visualizador)
-- ----------------------------------------------------------------
CREATE TABLE public.cxp_det_flete_cons (
    ...
    valor_cpdfc          NUMERIC(15, 2) NOT NULL,   -- Valor de la línea del XML emparejada a este envío
    observacion_cpdfc    VARCHAR(300),               -- Observación de esa línea del XML (trazabilidad)
    usuario_ingre        VARCHAR(50),
    hora_ingre           TIMESTAMP DEFAULT CURRENT_TIMESTAMP
);
