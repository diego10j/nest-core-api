...
BEGIN
    ...
    WHERE cci.ide_inepi = 1
      AND cci.ide_inbod = v_ide_inbod
      AND cci.fecha_trans_incci > v_fecha_corte_inccf
      AND cci.fecha_trans_incci <= p_fecha_conteo
      AND dci.ide_inarti = v_ide_inarti;
    ...
    v_valor_diferencia := v_diferencia * COALESCE(v_costo_unitario_actual, 0);
    ...
    costo_unitario := COALESCE(v_costo_unitario_actual, 0);
